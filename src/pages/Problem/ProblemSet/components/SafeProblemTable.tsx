...
const SafeProblemTable: React.FC = ()=>{
  ...
  //针对题目的操作
  const clickInspect = (entity: API.SafeQuestionVo) => {
    // alert(`/problemset/${entity.id}`)
    history.push(`/problemset/${entity.id}`)
  }

  //有关标签搜索的内容
  //初始化获取标签信息
  useEffect(() => {
    getProblemTagsUsingGET().then(res => {
      if(res.data){
        setOptions(res.data)
      }
    })
        ...
        <>
          {
            ...
            problem.status === '未开始' && <CloseCircleOutlined style={{fontSize: 18, color: Color.HARD}}/> ||
      ...
      render: (dom, entity) => {
        return (<>{
          <Tooltip placement="top" title="查看" color="#FA541C">
            <Button onClick={() => {
              clickInspect(entity)
            }} type="text" icon={<IconFont type='icon-chakan'/>}/>
          </Tooltip>
        }
        </>);
      },
