...
/** loginByEmail POST /api/user/login/email */
export async function loginByEmailUsingPOST(
  ...
  return request<API.BaseResponseUser_>('/api/user/login/email', {
    method: 'POST',
    headers: {
      'Content-Type': 'application/json',
    },
...
/** registerByEmail POST /api/user/register/email */
export async function registerByEmailUsingPOST(
  body: API.RegisterEmailRequest,
  options?: { [key: string]: any },
) {
  return request<API.BaseResponseLong_>('/api/user/register/email', {
    method: 'POST',
    headers: {
      'Content-Type': 'application/json',
    },
...
/** sendMsg POST /api/user/sendMsg/${param0} */
export async function sendMsgUsingPOST(
  // 叠加生成的Param类型 (非body参数swagger默认没有生成对象)
  params: API.sendMsgUsingPOSTParams,
  options?: { [key: string]: any },
) {
  const { email: param0, ...queryParams } = params;
  return request<API.BaseResponseString_>(`/api/user/sendMsg/${param0}`, {
    method: 'POST',
    params: { ...queryParams },
    ...(options || {}),
  });
